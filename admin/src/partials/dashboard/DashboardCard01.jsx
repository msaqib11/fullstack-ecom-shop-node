...
function DashboardCard01() {
  ...
  useEffect(() => {
    const monthlyRevenue = async () => {
      try {
        const response = await userRequest.get("/order/monthly-income");
        const rev = response.data[0].total - response.data[1].total
        setRevenue(rev);
        setRevenuePerc(Math.round((rev / response.data[1].total) * 100));
      } catch (error) {
    ...
    <div className="flex flex-col col-span-full sm:col-span-6 xl:col-span-4 bg-white dark:bg-gray-800 shadow-sm rounded-xl">
      <div className="px-5 pt-5">
        <header className="flex justify-between items-start mb-2">
          <h2 className="text-lg font-semibold text-gray-800 dark:text-gray-100 mb-2">Compared to last month</h2>
          {/* Menu button */}
        ...
        <div className="flex items-start">
          <div className="text-3xl font-bold text-gray-800 dark:text-gray-100 mr-2">${revenue}</div>
          {revenuePerc > 0 ?
            <div className="text-sm font-medium text-green-700 px-1.5 bg-green-500/20 rounded-full">↑ +{revenuePerc}%</div> :
            <div className="text-sm font-medium text-red-700 px-1.5 bg-red-500/20 rounded-full">↓ {revenuePerc}%</div>
          }
        </div>
      </div>
